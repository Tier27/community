...
*Setup instructions for a gitian build of COMMUNITY using a Debian VM or physical system.*
...
Clone the git repositories for community and gitian and then checkout the community version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the community repository
...
To build COMMUNITY (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the community repository.
...
```bash
    ...
    From https://github.com/COMMUNITYCRYPTO/COMMUNITY
...
For example:
```bash
URL=https://github.com/COMMUNITYCRYPTO/COMMUNITY
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/community-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/community-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/community-build.assert
```
...
[community/gitian.sigs](https://github.com/community-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
