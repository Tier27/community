...
/* *** BEGIN DARKSEND MAGIC - COMMUNITY **********
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                adds up to a cost of 0.001 COMMUNITY per transaction on average.
            */
            if (r <= 10) {
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeCOMMUNITYAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void CDarksendPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100COMMUNITY+1 ( bit on if present )
    // bit 1 - 10COMMUNITY+1
    // bit 2 - 1COMMUNITY+1
    // bit 3 - .1COMMUNITY+1
    // bit 3 - non-denom
...
void ThreadCheckDarKsendPool()
{
    ...
    RenameThread("community-Darksend");
