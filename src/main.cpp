...
#if defined(NDEBUG)
#error "COMMUNITY cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "COMMUNITY Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 0) return 300000 * COIN;//premine
		
	int64_t nSubsidy;
	
	if(nHeight >= 1 ) {
        nSubsidy = 100 * COIN;} 
	else if (nHeight > 500000 && nHeight <= 800200) { // 500000 => FIRST POS BLOCK
		nSubsidy = 75 * COIN;}
	else if (nHeight > 800200 ) {
	nSubsidy = 55 * COIN;}
    return nSubsidy;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	if(nHeight > 0 && nHeight <= 500000) {
		...
		if (mNodeCoins == 0) {
			...
			LogPrintf("[LIBRA] Adjusting Libra at height %d with %d masternodes (%d % locked COMMUNITY) and %d COMMUNITY supply at %ld\n", nHeight, nMasternodeCount, lockedCoinValue*100, nMoneySupply, GetTime());
